...
namespace myoddweb
{
  ...
  inline cancellation_token_source::cancellation_token_source() :
    _state(state::not_cancelled_sate)
  {
  }

  inline bool cancellation_token_source::is_cancellation_requested() const
  {
    return _state != state::not_cancelled_sate;
  }

  inline cancellation_token cancellation_token_source::token()
  {
    return cancellation_token(this);
  }

  inline void cancellation_token_source::cancel()
  {
  ...
  inline unsigned cancellation_token_source::register_callback(std::function<void()> callback)
  {
    std::unique_lock<std::mutex> lock_notify(_notify_mutex);
    if (is_cancellation_requested())
    {
  ...
  inline bool cancellation_token_source::unregister_callback(unsigned id)
  {
    std::unique_lock<std::mutex> lock_notify(_notify_mutex);
    if (is_cancellation_requested())
    {
  ...
  inline cancellation_token::cancellation_token(const cancellation_token& src) :
    _source(src._source)
  {
  }

  inline cancellation_token& cancellation_token::operator=(const cancellation_token& src)
  {
    if (this != &src)
    {
      _source = src._source;
  ...
  inline cancellation_token cancellation_token::none()
  {
    return cancellation_token(nullptr);
  }

  inline bool cancellation_token::is_cancellation_requested() const
  {
    return _source != nullptr && _source->is_cancellation_requested();
  }

  inline bool cancellation_token::can_be_cancelled() const
  {
    return _source != nullptr;
  }

  inline void cancellation_token::throw_if_cancellation_requested()
  {
    if (is_cancellation_requested())
    {
  ...
  inline cancellation_token::cancellation_token(cancellation_token_source* source) :
    _source(source)
  {
  }

  inline void cancellation_token::throw_operation_canceled_exception()
  {
    throw operation_canceled_exception();
  }

  inline unsigned cancellation_token::register_callback(std::function<void()> callback)
  {
  ...
  inline bool cancellation_token::unregister_callback(unsigned id)
  {
    //  if we have no source then it will never be called.
    if (_source == nullptr)
    {
